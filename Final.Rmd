...
You can find the [github repo](https://github.com/hjlee9295/edav_final) here.
...
geom_hex(bins = 20) +
  ...
  xlab("Score")+
  ggtitle("Heatmap of Initial Inspection scores of restaurants in NYC") +theme_grey(16) +scale_fill_viridis()
...
geom_point()  + 
  ...
  xlab("Mean Score")+
  ylab("") +
  labs(title="Initial inspection scores of restaurants\n in NYC by neighborhod")+  
...
ggplot(mean_score_ZIPCODE, aes(GRADE)) + 
  geom_bar(color = "black", fill = "lightblue") + 
  labs(title="Initial inspection grade counts \n
       by zipcode of restaurants in NYC") +
  xlab("Grades")+theme_grey(16)
```
...
ggplot(filter(df_rest_2, BORO!="0" & SCORE >=60), aes(SCORE)) + 
  ...
  ggtitle("Distribution of Restaurant's converted score by Borough,\n with values larger than 60") +
...
ggplot(df_count, aes(x=BORO, y= Freq, fill=GRADE)) + 
  ...
  ggtitle("Initial Inspection grades of restaurants by Borough")+theme_grey(16)+scale_fill_viridis(discrete = TRUE)
```
